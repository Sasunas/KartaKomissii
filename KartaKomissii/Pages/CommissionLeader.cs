        ...
        private void Delete_Click(object sender, EventArgs e)
        {
            ...
            if (result == System.Windows.Forms.DialogResult.Yes)
            {
                if (sotr.Checked == true)
                {
                    ...
                    string _command = "DELETE FROM Пароли WHERE ID_Сотрудника = '" + (dataGridView1.SelectedRows[0].Index + 1) + "'";
                    modules.Command(_command);
                    _command = "DELETE FROM [" + viewTableNow + "] WHERE ID = '" + (dataGridView1.SelectedRows[0].Index + 1) + "'";
                    modules.Command(_command);
                    _command = "UPDATE [" + viewTableNow + "] SET ID = ID - 1 WHERE ID > '" + (dataGridView1.SelectedRows[0].Index + 1) + "'";
                    modules.Command(_command);
                    modules.Close();
                }
                else
                {
                    ...
                    if (roli.Checked == true)
                    {
                        ...
                        string _command = "DELETE FROM [" + viewTableNow + "] WHERE ID = '" + (dataGridView1.SelectedRows[0].Index + 1) + "'";
                        modules.Command(_command);
                    }
                    else
                    {
                        ...
                        if (karty.Checked == true)
                        {
                            ...
                            string _command = "DELETE FROM [Оценка по требованию] WHERE ID_Карта_комиссии = '" + (dataGridView1.SelectedRows[0].Index + 1) + "'";
                            modules.Command(_command);
                            _command = "DELETE FROM [Роль в текущей карте] WHERE ID_Карты_комиссии = '" + (dataGridView1.SelectedRows[0].Index + 1) + "'";
                            modules.Command(_command);
                            _command = "DELETE FROM [" + viewTableNow + "] WHERE ID = '" + (dataGridView1.SelectedRows[0].Index + 1) + "'";
                            modules.Command(_command);
                        }
